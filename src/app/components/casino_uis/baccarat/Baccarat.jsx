        ...
        <div className="md:col-span-9 col-span-12 flex flex-col">
          <div className="grid grid-cols-4 gap-x-2">
            <div className="bg-[#2C3E50] px-6 whitespace-nowrap py-2 flex flex-col items-center w-full col-span-1 relative"
              onClick={() => handleUserSelection({ data: gameData.t2.find(item => item.nat === "Perferct Pair"), selection: "Perferct Pair" })}
            >
              {
                gameData && gameData.t2 && gameData.t2.find(item => item.nat === "Perferct Pair")?.gstatus == 0 && (
              ...
              <p className="text-xs md:text-md font-bold md:font-bold text-white">Perfect Pair</p>
              <p className='font-bold text-white text-md md:text-xl '>
                {gameData && gameData.t2 && gameData.t2.find(item => item.nat === 'Perferct Pair')?.gstatus != 0 ? (gameData.t2.find(item => item.nat === 'Perferct Pair')?.b1) : 0}:1
              </p>
            </div>

            <div className="bg-[#2C3E50] px-6 whitespace-nowrap py-2 flex flex-col items-center w-full col-span-1 relative"
              onClick={() => handleUserSelection({ data: gameData.t2.find(item => item.nat === "Big"), selection: "Big" })}
            >
              {
                gameData && gameData.t2 && gameData.t2.find(item => item.nat === "Big")?.gstatus == 0 && (
                  <div className="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center bg-black/[0.8] z-99">
                    <LockIcon fontSize='small' className="text-white" />
                  </div>
                )
              }
              <p className="text-xs md:text-md font-bold md:font-bold text-white">Big</p>
              <p className='font-bold text-white text-md md:text-xl '>
                {gameData && gameData.t2 && gameData.t2.find(item => item.nat === 'Big')?.gstatus != 0 ? (gameData.t2.find(item => item.nat === 'Big')?.b1) : 0}:1
              </p>
            </div>

            <div className="bg-[#2C3E50] px-6 whitespace-nowrap py-2 flex flex-col items-center w-full col-span-1 relative"
              onClick={() => handleUserSelection({ data: gameData.t2.find(item => item.nat === "Small"), selection: "Small" })}
            >
              {
                gameData && gameData.t2 && gameData.t2.find(item => item.nat === "Small")?.gstatus == 0 && (
                  <div className="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center bg-black/[0.8] z-99">
                    <LockIcon fontSize='small' className="text-white" />
                  </div>
                )
              }
              <p className="text-xs md:text-md font-bold md:font-bold text-white">Small</p>
              <p className='font-bold text-white text-md md:text-xl '>
                {gameData && gameData.t2 && gameData.t2.find(item => item.nat === 'Small')?.gstatus != 0 ? (gameData.t2.find(item => item.nat === 'Small')?.b1) : 0}:1
              </p>
            </div>

            <div className="bg-[#2C3E50] px-6 whitespace-nowrap py-2 flex flex-col items-center w-full col-span-1 relative"
              onClick={() => handleUserSelection({ data: gameData.t2.find(item => item.nat === "Either Pair"), selection: "Either Pair" })}
            >
              {
                gameData && gameData.t2 && gameData.t2.find(item => item.nat === "Either Pair")?.gstatus == 0 && (
                  <div className="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center bg-black/[0.8] z-99">
                    <LockIcon fontSize='small' className="text-white" />
                  </div>
                )
              }
              <p className="text-xs md:text-md font-bold md:font-bold text-white">Either Pair</p>
              <p className='font-bold text-white text-md md:text-xl '>
                {gameData && gameData.t2 && gameData.t2.find(item => item.nat === 'Either Pair')?.gstatus != 0 ? (gameData.t2.find(item => item.nat === 'Either Pair')?.b1) : 0}:1
              </p>
            </div>
          </div>
...
export default BaccaratInterface;
