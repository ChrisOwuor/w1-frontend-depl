...
const cardDataDragon = [
    { name: "Dragon Card A", image: "https://versionobj.ecoassetsservice.com/v21/static/front/img/cards/A.png" },
...
const cardDataTiger = [
    { name: "Tiger Card A", image: "https://versionobj.ecoassetsservice.com/v21/static/front/img/cards/A.png" },
...
const Card = ({ imageSrc, cardName, gameData, onClick }) => {
    const isLocked = gameData && gameData.t2 && gameData.t2.find(item => item.nat == cardName)?.gstatus == 0;
        ...
        <div className="flex justify-between bg-white mt-2">
            ...
            <div className="bg-white mt-2 p-10 flex-1">
                <div className="flex justify-center items-center w-full gap-x-2 mb-4">
                    ...
                    <p className="text-md font-bold text-black">
                        {gameData && gameData.t2 && gameData.t2.find(item => item.nat == "Dragon")?.gstatus === "1" ? "12" : "0"}
                    </p>
                </div>
                ...
                <div className="flex flex-wrap justify-center items-center gap-3 mb-3">
                    {cardDataDragon.map(card => (
                        <Card
                            key={card.name}
                            onClick={() => handleUserSelection({ data: gameData?.t2?.find(item => item.nat === card.name), selection: card.name })}
                            imageSrc={card.image}
                            cardName={card.name}
                            gameData={gameData}
                        />
            ...
            <div className="bg-white mt-2 p-10 flex-1">
                <div className="flex justify-center items-center w-full gap-x-2 mb-4">
                    ...
                    <p className="text-md font-bold text-black">
                        {gameData && gameData.t2 && gameData.t2.find(item => item.nat == "Tiger")?.gstatus === "1" ? "12" : "0"}
                    </p>
                </div>
