...
const TeenpattiTest = () => {
  ...
  const handleUserSelection = ({ data, selection }) => {
    if(data.lstatus == "False" || data.dstatus == "False"|| data.tstatus == "False"){
      return
    }
    setBet(prev => ({
      ...prev,
  ...
  return (
    <>
          ...
          <div className="relative flex flex-col gap-x-2 bg-black h-[27vh] sm:h-[50vh]">
            ...
            <div className="grid grid-cols-12 h-full relative">
              <div className="col-span-2 md:col-span-3 max-md:absolute top-0 left-0 z-999 p-2">
                ...
                <div className="flex flex-col">
                  <p className='sm:text-md text-sm font-bold'>LION</p>
                ...
                <div className="flex flex-col">
                  <p className='sm:text-md text-sm font-bold'>DRAGON</p>
                  <div className="flex items-center gap-x-1">
                    <img src={`https://versionobj.ecoassetsservice.com/v19/static/front/img/cards/${gameData && gameData.t1[0].C7 || "1"}.jpg`} className="w-4.5 sm:w-9" />
                    <img src={`https://versionobj.ecoassetsservice.com/v19/static/front/img/cards/${gameData && gameData.t1[0].C8 || "1"}.jpg`} className="w-4.5 sm:w-9" />
                    <img src={`https://versionobj.ecoassetsservice.com/v19/static/front/img/cards/${gameData && gameData.t1[0].C9 || "1"}.jpg`} className="w-4.5 sm:w-9" />
                  </div>
                </div>
      ...
      <div className="grid grid-cols-5 bg-white mx-1 border border-black/[0.3]">
        {/* Header */}
        <div className={`col-span-2 px-3 py-2 flex justify-center border-l border-r border-black/[0.3]`}>
          {/*  */}
        </div>
        <div className={`col-span-1 px-3 py-2 bg-back flex justify-center border-l border-r border-black/[0.3]`}>
          <p className="text-black text-md font-bold">Tiger</p>
        </div>
        <div className={`col-span-1 px-3 py-2 bg-back flex justify-center border-l border-r border-black/[0.3]`}>
          <p className="text-black text-md font-bold">Lion</p>
        </div>
        <div className={`col-span-1 px-3 py-2 bg-back flex justify-center border-l border-r border-black/[0.3]`}>
          <p className="text-black text-md font-bold">Dragon</p>
        </div>
      </div>

      {
        ["Winner", "Pair", "Flush", "Straight", "Trio", "Straight Flush"].map((runner, i) => {
          return (
            <div className="grid grid-cols-5 bg-white mx-1 border border-black/[0.3]" key={i}>
              {/* First Column */}
              <div className="col-span-2 px-3 py-2 flex items-center justify-start border-r border-black/[0.3]">
                <p className="text-black text-md font-bold">{runner}</p>
              </div>

              {/* Tiger Column */}
              <div
                className={`col-span-1 px-3 py-2 bg-back flex justify-center border-l border-r border-black/[0.3] ${gameData && gameData.t2 && gameData.t2.find(item => item.nation === runner)?.tstatus === "False" ? "relative" : ""}`}
                onClick={() => handleUserSelection({
                  data: {
                    ...gameData.t2.find(item => item.nation === runner),
                    rate: gameData.t2.find(item => item.nation === runner).trate,
                  },
                  selection: `Tiger ${runner}`,
                })}
              >
                {gameData && gameData.t2 && gameData.t2.find(item => item.nation === runner)?.tstatus === "False" && (
                ...
                <p className="font-bold text-black text-md md:text-xl">
                  {gameData && gameData.t2 && gameData.t2.find(item => item.nation === runner)?.trate}
                </p>
              </div>

              {/* Lion Column */}
              <div
                className={`col-span-1 px-3 py-2 bg-back flex justify-center border-l border-r border-black/[0.3] ${gameData && gameData.t2 && gameData.t2.find(item => item.nation === runner)?.lstatus === "False" ? "relative" : ""}`}
                onClick={() => handleUserSelection({
                  data: {
                    ...gameData.t2.find(item => item.nation === runner),
                    rate: gameData.t2.find(item => item.nation === runner).lrate,
                  },
                  selection: `Lion ${runner}`,
                })}
              >
                {gameData && gameData.t2 && gameData.t2.find(item => item.nation === runner)?.lstatus === "False" && (
                  <div className="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center bg-black/[0.8] z-10">
                    <LockIcon fontSize="small" className="text-white" />
                  </div>
                )}
                <p className="font-bold text-black text-md md:text-xl">
                  {gameData && gameData.t2 && gameData.t2.find(item => item.nation === runner)?.lrate}
                </p>
              </div>

              {/* Dragon Column */}
              <div
                className={`col-span-1 px-3 py-2 bg-back flex justify-center border-l border-black/[0.3] ${gameData && gameData.t2 && gameData.t2.find(item => item.nation === runner)?.dstatus === "False" ? "relative" : ""}`}
                onClick={() => handleUserSelection({
                  data: {
                    ...gameData.t2.find(item => item.nation === runner),
                    rate: gameData.t2.find(item => item.nation === runner).drate,
                  },
                  selection: `Dragon ${runner}`,
                })}
              >
                {gameData && gameData.t2 && gameData.t2.find(item => item.nation === runner)?.dstatus === "False" && (
                  <div className="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center bg-black/[0.8] z-10">
                    <LockIcon fontSize="small" className="text-white" />
                  </div>
                )}
                <p className="font-bold text-black text-md md:text-xl">
                  {gameData && gameData.t2 && gameData.t2.find(item => item.nation === runner)?.drate}
                </p>
              </div>
            </div>
          );
